...
export function ReportExportsPage({ client, navigation }: { client: ReportCenterClient; navigation?: ReactNode }) {
	...
	return <PageCanvas>{navigation}<PageHeader eyebrow="EXPORT ARCHIVE" title="导出中心" description="查看正式 Excel、文件留存与 Oracle 结果清理状态。" actions={<button type="button" onClick={() => void load()} disabled={state.loading}><RefreshCw aria-hidden="true" />刷新</button>} /><FilterToolbar summary={<StatusTag tone="neutral">{items.length} 个任务</StatusTag>}><div className={styles.filters}><label>任务状态<select value={status} onChange={(event) => setStatus(event.currentTarget.value)}><option value="">全部状态</option>{['PENDING','RUNNING','READY','FAILED','CANCELLED','EXPIRED'].map((value) => <option key={value} value={value}>{exportLabel(value as ReportExportStatus)}</option>)}</select></label></div></FilterToolbar><Section title="导出任务" description="一个运行只生成一个正式 Excel；文件就绪后复用 OSS，Oracle 结果按 run_id 清理。" flush>
