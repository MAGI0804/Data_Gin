...
import { DataTable, FeedbackState, FilterToolbar, MetricStrip, PageCanvas, PageHeader, Section, StatusTag, type StatusTagTone } from '../../../ui'
...
export function ReportExportsPage({ client, navigation }: { client: ReportCenterClient; navigation?: ReactNode }) {
	...
	return <PageCanvas density="compact">{navigation}<PageHeader eyebrow="EXPORT ARCHIVE" title="导出中心" description="查看正式 Excel、文件留存与 Oracle 结果清理状态。" actions={<button type="button" onClick={() => void load()} disabled={state.loading}><RefreshCw aria-hidden="true" />刷新</button>} /><MetricStrip label="当前加载的导出任务概览" items={[{ key: 'total', label: '已加载', value: items.length, detail: hasMore ? '仍有更多' : '当前全部' }, { key: 'running', label: '处理中', value: items.filter((item) => item.status === 'PENDING' || item.status === 'RUNNING').length, detail: '等待或生成中' }, { key: 'ready', label: '可下载', value: items.filter((item) => item.canDownload).length, detail: '文件就绪' }, { key: 'issues', label: '需关注', value: items.filter((item) => item.status === 'FAILED' || item.status === 'CANCELLED' || item.status === 'EXPIRED').length, detail: '失败 / 取消 / 过期' }]} /><FilterToolbar summary={<StatusTag tone="neutral">{items.length} 个任务</StatusTag>}><div className={styles.filters}><label>任务状态<select name="reportExportStatus" value={status} onChange={(event) => setStatus(event.currentTarget.value)}><option value="">全部状态</option>{['PENDING','RUNNING','READY','FAILED','CANCELLED','EXPIRED'].map((value) => <option key={value} value={value}>{exportLabel(value as ReportExportStatus)}</option>)}</select></label></div></FilterToolbar><Section title="导出任务" description="一个运行只生成一个正式 Excel；文件就绪后复用 OSS，Oracle 结果按报表编号清理。" flush>
