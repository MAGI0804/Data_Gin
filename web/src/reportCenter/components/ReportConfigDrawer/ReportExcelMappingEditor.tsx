...
export function ReportExcelMappingEditor({ columns, onChange }: { columns: ReportColumn[]; onChange: (columns: ReportColumn[]) => void }) {
  ...
  return <div className={styles.editor}>
    <JSONDocumentEditor label="Excel 字段映射 JSON" description={'键是 Oracle REF CURSOR 返回字段，值是 Excel 表头，例如 {"a":"id"}。'} value={mapping} parse={parseExcelMappingDocument} onChange={setMapping} />
    ...
    <div className={styles.rows}>
      {entries.map(([databaseColumn, excelHeader], index) => <div className={styles.row} key={index}>
        <label><span>Oracle 游标字段</span><input className={styles.mono} value={databaseColumn} onChange={(event) => updateField(databaseColumn, event.currentTarget.value)} /></label>
      ...
      {entries.length === 0 ? <p className={styles.empty}>可先粘贴字段映射 JSON，或手动新增一行；执行样例探测后也可由游标字段自动生成。</p> : null}
    </div>
  </div>
}
