...
export function MallWeatherExportProfilePanel({ client }: { client: ProfileApiClient }) {
  ...
  return <section className="workbench-panel mall-weather-export-profiles" aria-busy={loading || saving}>
    <div className="mall-weather-section-title">
      <div><strong>天气导出 Profile</strong><span>维护可复用导出配置；系统固定 Profile 仅供读取，不能在此修改。</span></div>
      <div className="mall-weather-inline-actions">
        <button type="button" onClick={() => void loadProfiles()} disabled={loading || saving}><RefreshCcw aria-hidden="true" />刷新</button>
        <button className="primary" type="button" onClick={startCreate} disabled={listForbidden || saving}><Plus aria-hidden="true" />新建 Profile</button>
      </div>
    </div>
    ...
    {profiles.length > 0 && <div className="data-table-wrap"><table className="data-table"><caption>天气导出 Profile 列表</caption><thead><tr><th>名称</th><th>编码</th><th>版本</th><th>状态</th><th>数据集</th><th>更新时间</th><th>操作</th></tr></thead><tbody>{profiles.map((profile) => <tr key={profile.id}><td>{profile.name}</td><td>{profile.code}</td><td>{profile.version}</td><td>{profile.enabled ? '已启用' : '已停用'}</td><td>{profile.datasets.length}</td><td>{formatDate(profile.updatedAt)}</td><td><button type="button" onClick={() => startEdit(profile)} disabled={saving || mallWeatherExportProfileReadOnly(profile.code)}>{mallWeatherExportProfileReadOnly(profile.code) ? '系统固定' : '编辑'}</button></td></tr>)}</tbody></table></div>}
    {!loading && !error && profiles.length === 0 && <p className="mall-weather-action-message" role="status">暂无符合条件的天气导出 Profile。</p>}
    {nextCursor && <button type="button" onClick={() => void loadProfiles(nextCursor, true)} disabled={loading || saving}>加载更多</button>}

    {formOpen && <form className="mall-weather-profile-form" onSubmit={save}>
      <div className="mall-weather-section-title"><div><strong>{editing ? `编辑 Profile · v${editing.version}` : '新建 Profile'}</strong><span>保存为完整配置；更新将采用乐观锁，避免覆盖他人修改。</span></div><button type="button" onClick={cancelEdit} disabled={saving} aria-label="关闭 Profile 编辑"><X aria-hidden="true" />关闭</button></div>
      ...
      <label className="mall-weather-checkbox"><input type="checkbox" checked={form.enabled} onChange={(event) => update('enabled', event.currentTarget.checked)} disabled={saving} />启用此 Profile</label>

      <details className="mall-weather-profile-details"><summary>筛选条件</summary><div className="mall-weather-profile-form mall-weather-profile-nested"><label><span>商场 ID（逗号分隔）</span><input value={form.mallIds} onChange={(event) => update('mallIds', event.currentTarget.value)} disabled={saving} inputMode="numeric" /></label><label><span>城市（逗号分隔）</span><input value={form.cities} onChange={(event) => update('cities', event.currentTarget.value)} disabled={saving} /></label><label><span>商场状态</span><input value={form.mallStatuses} onChange={(event) => update('mallStatuses', event.currentTarget.value)} disabled={saving} placeholder="draft,active,disabled" /></label><label><span>质量状态</span><input value={form.qualityStatuses} onChange={(event) => update('qualityStatuses', event.currentTarget.value)} disabled={saving} placeholder="valid,warning" /></label><label><span>开始时间（RFC3339）</span><input value={form.start} onChange={(event) => update('start', event.currentTarget.value)} disabled={saving} placeholder="2026-01-02T03:04:05Z" /></label><label><span>结束时间（RFC3339）</span><input value={form.end} onChange={(event) => update('end', event.currentTarget.value)} disabled={saving} placeholder="2026-01-03T03:04:05Z" /></label></div></details>

      <div className="mall-weather-profile-datasets"><div><strong>数据集 *</strong><span>未指定列时由服务端使用该数据集的默认列。</span></div>{form.datasets.map((dataset, index) => <div className="mall-weather-profile-dataset" key={`${dataset.kind}-${index}`}><label><span>类型</span><select value={dataset.kind} onChange={(event) => replaceDatasetKind(index, event.currentTarget.value)} disabled={saving}>{mallWeatherExportProfileDatasetKinds.map((kind) => <option value={kind} key={kind}>{datasetLabel(kind)}</option>)}</select></label><label><span>工作表名称 *</span><input value={dataset.sheetName} onChange={(event) => updateDataset(index, 'sheetName', event.currentTarget.value)} maxLength={31} required disabled={saving} /></label><label className="mall-weather-checkbox"><input type="checkbox" checked={dataset.freezeHeader} onChange={(event) => updateDataset(index, 'freezeHeader', event.currentTarget.checked)} disabled={saving} />冻结表头</label><label className="mall-weather-checkbox"><input type="checkbox" checked={dataset.autoFilter} onChange={(event) => updateDataset(index, 'autoFilter', event.currentTarget.checked)} disabled={saving} />自动筛选</label><button type="button" onClick={() => setForm((current) => ({ ...current, datasets: current.datasets.filter((_, currentIndex) => currentIndex !== index) }))} disabled={saving || form.datasets.length === 1}>移除</button></div>)}<button type="button" onClick={() => setForm((current) => current.datasets.length >= 8 ? current : ({ ...current, datasets: [...current.datasets, newMallWeatherExportProfileDataset()] }))} disabled={saving || form.datasets.length >= 8}><Plus aria-hidden="true" />添加数据集</button></div>
      <div className="mall-weather-form-actions"><button className="primary" type="submit" disabled={saving || listForbidden}><Save aria-hidden="true" />{saving ? '保存中' : editing ? '保存更新' : '创建 Profile'}</button></div>
    </form>}
