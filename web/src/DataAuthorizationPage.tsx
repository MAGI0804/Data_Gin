...
export function DataAuthorizationPage({ client }: { client: ApiClient }) {
  ...
  const loadAccounts = useCallback(async (options: { append?: boolean; search?: string } = {}) => {
    ...
    setLoading(false)
    if (!response.ok) {
      setError(response.status === 403 ? '当前登录账号不是可信管理员，无法管理数据授权。' : dataAuthorizationMessage(response.data, '开放接口账号加载失败，请稍后重试。'))
      return
    }
  ...
  async function createAccount(event: FormEvent<HTMLFormElement>) {
    ...
    setMutating(true)
    setError('')
    const response = await client('/v1/data-authorizations/accounts/create', {
      method: 'POST',
    ...
    setMutating(false)
    if (!response.ok) {
      setError(dataAuthorizationMessage(response.data, '账号开通失败，请检查字段和授权有效期。'))
      return
    }
  ...
  async function submitAction(event: FormEvent<HTMLFormElement>) {
    ...
    if (actionDialog.kind === 'grant') {
      const expiresAt = authorizationExpiryISO(formString(form, 'expiresAt'))
      path = `/v1/data-authorizations/accounts/${selected.id}/permissions/grant`
      body = { ...body, permission: actionDialog.permission, expiresAt }
      scope = 'permission-grant'
    } else if (actionDialog.kind === 'revoke') {
      path = `/v1/data-authorizations/accounts/${selected.id}/permissions/revoke`
      body = { ...body, permission: actionDialog.permission }
      scope = 'permission-revoke'
      confirmation = `确认撤销 ${selected.account} 的 ${permissionLabel(actionDialog.permission)}？撤销后下一次开放接口请求立即失效。`
    } else {
      path = `/v1/data-authorizations/accounts/${selected.id}/token/reissue`
      scope = 'token-reissue'
      confirmation = `确认重新签发 ${selected.account} 的访问 Token？旧 Token 将立即失效。`
    }
    ...
    setMutating(false)
    if (!response.ok) {
      setError(dataAuthorizationMessage(response.data, '数据授权操作失败，请刷新后重试。'))
      return
    }
    if (actionDialog.kind === 'reissue') {
      const data = responseData(response.data)
      const token = typeof data?.token === 'string' ? data.token : ''
      if (token) setOneTimeToken(token)
    }
    const message = actionDialog.kind === 'grant' ? '授权已保存。' : actionDialog.kind === 'revoke' ? '权限已撤销。' : '访问 Token 已重新签发。'
    setActionDialog(null)
    await refreshAfterMutation(message, selected.id)
  }
  ...
  return (
    <div className="view-stack data-authorization-page" aria-busy={loading || mutating}>
      ...
      {createOpen && <Modal title="开通开放 API 账号" onClose={() => setCreateOpen(false)}><form className="data-authorization-form" onSubmit={createAccount}>
        ...
        <div className="data-authorization-form-actions wide"><button type="button" onClick={() => setCreateOpen(false)}>取消</button><button className="primary" type="submit" disabled={mutating}>{mutating ? '正在开通…' : '确认开通'}</button></div>
      </form></Modal>}

      {actionDialog && selected && <Modal title={actionTitle(actionDialog)} onClose={() => setActionDialog(null)}><form className="data-authorization-form single" onSubmit={submitAction}>
        ...
        <div className="data-authorization-form-actions"><button type="button" onClick={() => setActionDialog(null)}>取消</button><button className={actionDialog.kind === 'revoke' ? 'danger' : 'primary'} type="submit" disabled={mutating}>{mutating ? '正在提交…' : '继续'}</button></div>
      </form></Modal>}
...
function Modal({ title, children, onClose }: { title: string; children: React.ReactNode; onClose: () => void }) {
  return <div className="modal-backdrop" role="presentation"><section className="modal-panel data-authorization-modal" role="dialog" aria-modal="true" aria-label={title}><div className="modal-title"><h3>{title}</h3><button type="button" onClick={onClose}>关闭</button></div><div className="modal-body">{children}</div></section></div>
}
