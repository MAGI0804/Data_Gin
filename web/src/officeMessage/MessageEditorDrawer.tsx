...
export function MessageEditorDrawer({ client, draft, saving, error, onChange, onClose, onSave, onNotice }: Props) {
  function switchSource(sourceType: OfficeMessageSourceType) {
    if (!draft) return
    onChange({ ...draft, sourceType, content: sourceType === 'EDITED' ? draft.content : '', parameters: sourceType === 'ORACLE_QUERY' ? draft.parameters : [], columnMapping: sourceType === 'EDITED' ? [] : draft.columnMapping })
  }
