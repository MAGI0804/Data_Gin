...
type ComposerClient = (path: string, options: { method: HTTPMethod; body?: unknown; signal?: AbortSignal; showResult: false; silentLoading: true }) => Promise<ClientResponse>
...
function StageTypeLabel({ value }: { value: StageType }) {
  ...
  return <span className="pipeline-stage-kind">{labels[value]}</span>
}

export function PipelineComposerPanel({ pipelines, client, onRefresh }: { pipelines: PipelineSummary[]; client: ComposerClient; onRefresh: () => void }) {
  ...
  async function savePipeline(event: FormEvent<HTMLFormElement>) {
    ...
    if (!pipelineDraft || saving) return
    ...
    setSaving(true)
    const response = await client(pipelineDraft.id ? pipelinePath(pipelineDraft.id) : '/v1/pipelines', {
      method: pipelineDraft.id ? 'PUT' : 'POST', showResult: false, silentLoading: true,
      body: { name, code, description: pipelineDraft.description.trim(), enabled: pipelineDraft.enabled },
    })
    setSaving(false)
    if (!response.ok) { setMessage(safeMessage(response.status, '保存流水线')); return }
    setPipelineDraft(null)
    setMessage('流水线已保存。')
    onRefresh()
    const created = parsePipelineWriteResult(response.data)
    const pipelineID = pipelineDraft.id ?? created?.id
    if (pipelineID) setSelectedID(pipelineID)
  }

  async function saveStage(event: FormEvent<HTMLFormElement>) {
    event.preventDefault()
    if (!detail || detail.pipeline.id !== selected?.id || !stageDraft || saving) return
    ...
    setSaving(true)
    const response = await client(stageDraft.id ? pipelineStagePath(stageDraft.id) : `${pipelinePath(detail.pipeline.id)}/stages`, {
      method: stageDraft.id ? 'PUT' : 'POST', showResult: false, silentLoading: true,
      body: { stage_type: stageDraft.stageType, name: stageDraft.name.trim(), order_index: orderIndex, enabled: stageDraft.enabled },
    })
    setSaving(false)
    if (!response.ok) { setMessage(safeMessage(response.status, '保存阶段')); return }
    setStageDraft(null)
    setMessage('阶段已保存。')
    refreshAfterWrite(detail.pipeline.id)
  }

  async function saveStep(event: FormEvent<HTMLFormElement>) {
    event.preventDefault()
    if (!detail || detail.pipeline.id !== selected?.id || !stepDraft || saving || stepDraft.hasMaskedSecret) return
    ...
    const response = await client(path, {
      method: stepDraft.id ? 'PUT' : 'POST', showResult: false, silentLoading: true,
      body: { stage_id: stage.stage.id, code: stepDraft.code.trim(), name: stepDraft.name.trim(), method_type: stepDraft.methodType, order_index: orderIndex, enabled: stepDraft.enabled, timeout_seconds: timeoutSeconds, params: params as MethodParam[], outputs: outputs as MethodOutput[] },
    })
    setSaving(false)
    if (!response.ok) { setMessage(safeMessage(response.status, '保存步骤')); return }
    setStepDraft(null)
    setMessage('步骤已保存。')
    refreshAfterWrite(detail.pipeline.id)
  }
  ...
  async function generateStageConfig(stage: PipelineStageDetail) {
    if (!detail || detail.pipeline.id !== selected?.id || saving) return
    setSaving(true)
    ...
    setSaving(false)
    if (!parseStageGeneratedConfig(response.data)) { setMessage(safeMessage(response.status, '生成阶段配置')); return }
    setMessage(`已生成“${stage.stage.name}”的阶段配置，尚未发布。`)
    refreshAfterWrite(detail.pipeline.id)
  }

  async function publishStageConfig(stage: PipelineStageDetail) {
    if (!detail || detail.pipeline.id !== selected?.id || saving || publishingStageID !== stage.stage.id || !publishAcknowledged) return
    setSaving(true)
    ...
    setPublishingStageID(null)
    setPublishAcknowledged(false)
    const config = parseStageGeneratedConfig(response.data)
    if (!config) { setMessage(safeMessage(response.status, '发布阶段配置')); return }
    setMessage(config.target_ref_id > 0 ? `阶段已发布，已关联 ${config.target_ref_type} #${config.target_ref_id}。` : '日志阶段配置已发布。')
    refreshAfterWrite(detail.pipeline.id)
  }

  return <section className="pipeline-composer" aria-busy={loading || saving}>
    <div className="pipeline-composer-head">
      <div><strong>流水线编排</strong><span>创建、编辑、预览、生成与发布均使用真实接口；发布会创建或关联真实配置资源。</span></div>
      <div className="pipeline-composer-actions"><button type="button" onClick={() => { setMessage(''); setPipelineDraft(emptyPipelineDraft()) }} disabled={saving}>新增流水线</button>{activeDetail && <><button type="button" onClick={() => setPipelineDraft({ id: activeDetail.pipeline.id, name: activeDetail.pipeline.name, code: activeDetail.pipeline.code, description: activeDetail.pipeline.description ?? '', enabled: activeDetail.pipeline.enabled })} disabled={saving}>编辑流水线</button><button type="button" onClick={() => void loadPreview()} disabled={saving || loading}>预览 JSON</button></>}</div>
    </div>
    {message && <p className="pipeline-composer-message" role="status">{message}</p>}
    {pipelines.length === 0 ? <p role="status">尚无流水线，请先创建。</p> : <div className="pipeline-composer-layout">
      ...
      <div className="pipeline-stage-workspace">
        {loading && !activeDetail ? <p role="status">正在加载流水线详情…</p> : activeDetail && <>
          <div className="pipeline-detail-title"><strong>{activeDetail.pipeline.name}</strong><span>{activeDetail.pipeline.description || '未填写说明'} / {activeDetail.pipeline.enabled ? '已启用' : '已停用'}</span>{availableStageTypes.length > 0 && <button type="button" onClick={() => setStageDraft({ id: null, stageType: availableStageTypes[0], name: '', orderIndex: String(activeDetail.stages.length + 1), enabled: true })} disabled={saving}>补充阶段</button>}</div>
          {activeDetail.stages.length === 0 ? <p role="status">暂无阶段。</p> : activeDetail.stages.map((stage) => <article className="pipeline-stage" key={stage.stage.id}>
            <div className="pipeline-stage-head"><div><StageTypeLabel value={stage.stage.stage_type} /><strong>{stage.stage.name}</strong><span>顺序 {stage.stage.order_index} / {stage.stage.enabled ? '启用' : '停用'} / {stage.steps.length} 个步骤</span></div><div className="pipeline-composer-actions"><button type="button" onClick={() => setStageDraft(stageDraftFrom(stage.stage))} disabled={saving}>编辑</button><button type="button" onClick={() => setStepDraft(emptyStepDraft(stage.stage))} disabled={saving}>新增步骤</button><button type="button" onClick={() => void generateStageConfig(stage)} disabled={saving}>生成配置</button></div></div>
            {stage.generated_config && <div className="pipeline-generated-config"><span>配置版本 {stage.generated_config.version} / {stage.generated_config.target_ref_id > 0 ? `已发布至 ${stage.generated_config.target_ref_type} #${stage.generated_config.target_ref_id}` : '未发布'}</span>{publishingStageID === stage.stage.id ? <label className="pipeline-publish-confirm"><input type="checkbox" checked={publishAcknowledged} onChange={(event) => setPublishAcknowledged(event.currentTarget.checked)} />我确认发布会创建或关联真实资源。<button className="danger" type="button" onClick={() => void publishStageConfig(stage)} disabled={saving || !publishAcknowledged}>确认发布</button><button type="button" onClick={() => { setPublishingStageID(null); setPublishAcknowledged(false) }} disabled={saving}>取消</button></label> : <button className="danger" type="button" onClick={() => { setPublishingStageID(stage.stage.id); setPublishAcknowledged(false) }} disabled={saving || stage.generated_config.target_ref_id > 0}>{stage.generated_config.target_ref_id > 0 ? '已发布' : '发布配置'}</button>}</div>}
            {stage.steps.length === 0 ? <p className="pipeline-empty-step">暂无步骤。</p> : <div className="pipeline-step-list">{stage.steps.map((item) => <div className="pipeline-step" key={item.step.id}><div><strong>{item.step.name}</strong><span>{item.step.code} · {item.step.method_type} · 超时 {item.step.timeout_seconds} 秒 · {item.step.enabled ? '启用' : '停用'}</span></div><button type="button" onClick={() => setStepDraft(stepDraftFrom(item))} disabled={saving}>编辑步骤</button></div>)}</div>}
          </article>)}
        </>}
      </div>
    </div>}
    {pipelineDraft && <form className="pipeline-editor" onSubmit={savePipeline}><h4>{pipelineDraft.id ? '编辑流水线' : '新增流水线'}</h4><label>名称<input value={pipelineDraft.name} required maxLength={100} onChange={(event) => setPipelineDraft({ ...pipelineDraft, name: event.currentTarget.value })} /></label><label>编码<input value={pipelineDraft.code} required maxLength={100} onChange={(event) => setPipelineDraft({ ...pipelineDraft, code: event.currentTarget.value })} /></label><label>说明<textarea rows={3} value={pipelineDraft.description} onChange={(event) => setPipelineDraft({ ...pipelineDraft, description: event.currentTarget.value })} /></label><label className="checkbox-label"><input type="checkbox" checked={pipelineDraft.enabled} onChange={(event) => setPipelineDraft({ ...pipelineDraft, enabled: event.currentTarget.checked })} />启用流水线</label><div className="pipeline-composer-actions"><button type="button" onClick={() => setPipelineDraft(null)} disabled={saving}>取消</button><button className="primary" type="submit" disabled={saving}>{saving ? '保存中…' : '保存'}</button></div></form>}
    {stageDraft && <form className="pipeline-editor" onSubmit={saveStage}><h4>{stageDraft.id ? '编辑阶段' : '补充阶段'}</h4><label>阶段类型<select value={stageDraft.stageType} onChange={(event) => setStageDraft({ ...stageDraft, stageType: event.currentTarget.value as StageType })} disabled={Boolean(stageDraft.id)}>{(stageDraft.id ? [stageDraft.stageType] : availableStageTypes).map((kind) => <option key={kind} value={kind}>{kind}</option>)}</select></label><label>名称<input value={stageDraft.name} required maxLength={100} onChange={(event) => setStageDraft({ ...stageDraft, name: event.currentTarget.value })} /></label><label>顺序<input type="number" min="0" value={stageDraft.orderIndex} onChange={(event) => setStageDraft({ ...stageDraft, orderIndex: event.currentTarget.value })} /></label><label className="checkbox-label"><input type="checkbox" checked={stageDraft.enabled} onChange={(event) => setStageDraft({ ...stageDraft, enabled: event.currentTarget.checked })} />启用阶段</label><div className="pipeline-composer-actions"><button type="button" onClick={() => setStageDraft(null)} disabled={saving}>取消</button><button className="primary" type="submit" disabled={saving}>{saving ? '保存中…' : '保存'}</button></div></form>}
    {stepDraft && <form className="pipeline-editor" onSubmit={saveStep}><h4>{stepDraft.id ? '编辑步骤' : '新增步骤'}</h4>{stepDraft.hasMaskedSecret && <p className="pipeline-composer-error" role="alert">该步骤含有已隐藏的密钥；为防止保存时覆盖真实值，当前不允许编辑。请在后端提供保留密钥的更新契约后再操作。</p>}<label>编码<input value={stepDraft.code} required maxLength={100} onChange={(event) => setStepDraft({ ...stepDraft, code: event.currentTarget.value })} /></label><label>名称<input value={stepDraft.name} required maxLength={100} onChange={(event) => setStepDraft({ ...stepDraft, name: event.currentTarget.value })} /></label><label>方法类型<select value={stepDraft.methodType} onChange={(event) => setStepDraft({ ...stepDraft, methodType: event.currentTarget.value })}>{activeDetail?.stages.find((stage) => stage.stage.id === stepDraft.stageID) && stageMethodTypes(activeDetail.stages.find((stage) => stage.stage.id === stepDraft.stageID)!.stage.stage_type).map((kind) => <option key={kind} value={kind}>{kind}</option>)}</select></label><label>顺序<input type="number" min="0" value={stepDraft.orderIndex} onChange={(event) => setStepDraft({ ...stepDraft, orderIndex: event.currentTarget.value })} /></label><label>超时秒数<input type="number" min="1" max="86400" value={stepDraft.timeoutSeconds} onChange={(event) => setStepDraft({ ...stepDraft, timeoutSeconds: event.currentTarget.value })} /></label><label className="checkbox-label"><input type="checkbox" checked={stepDraft.enabled} onChange={(event) => setStepDraft({ ...stepDraft, enabled: event.currentTarget.checked })} />启用步骤</label><label>参数 JSON<textarea rows={8} value={stepDraft.paramsJSON} onChange={(event) => setStepDraft({ ...stepDraft, paramsJSON: event.currentTarget.value })} /></label><label>输出 JSON<textarea rows={8} value={stepDraft.outputsJSON} onChange={(event) => setStepDraft({ ...stepDraft, outputsJSON: event.currentTarget.value })} /></label><p className="query-contract-note">参数需要 location、name、value_source；输出需要 name。秘密参数只在新建时填写，且不会从服务端回显。</p><div className="pipeline-composer-actions"><button type="button" onClick={() => setStepDraft(null)} disabled={saving}>取消</button><button className="primary" type="submit" disabled={saving || stepDraft.hasMaskedSecret}>{saving ? '保存中…' : '保存'}</button></div></form>}
    {preview && <pre className="pipeline-preview" aria-label="流水线 JSON 预览">{JSON.stringify(preview, null, 2)}</pre>}
  </section>
}
