...
func TestBuildFetchRunQueryUsesFiltersAndDescendingKeyset(t *testing.T) {
	...
	statement, args, err := buildFetchRunQuery(FetchRunQuery{
		...
		TaskKind: "mall:weather:full", EndpointKind: "v26_weather", Status: "partial_success",
	...
	for _, fragment := range []string{
		"r.mall_id = ?", "r.created_at >= ?", "r.created_at < ?", "r.task_kind = ?",
	...
	if len(args) != 10 || strings.Contains(statement, "mall:weather:full") || strings.Contains(statement, "partial_success") {
