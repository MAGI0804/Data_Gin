...
func TestProcessExcelMatchFilePreservesSourceCellTypesAndStyles(t *testing.T) {
	dir := t.TempDir()
	inputPath := filepath.Join(dir, "source.xlsx")
	outputPath := filepath.Join(dir, "result.xlsx")
	...
	if err := f.SetSheetRow(sheet, "A2", &[]interface{}{"杭州恒隆", "B001", 123.45}); err != nil {
		t.Fatal(err)
	}
	...
	if err := f.SetCellStyle(sheet, "C2", "C2", amountStyle); err != nil {
		t.Fatal(err)
	}
	if err := f.SaveAs(inputPath); err != nil {
		t.Fatalf("SaveAs failed: %v", err)
	...
	_, err = processExcelMatchFile(context.Background(), inputPath, outputPath, cfg, &fakeExcelMatchLookup{value: map[string]string{"B001": "杭州恒隆店"}})
	if err != nil {
		t.Fatalf("processExcelMatchFile failed: %v", err)
	}
	...
	if cellType == excelize.CellTypeInlineString || cellType == excelize.CellTypeSharedString {
		t.Fatalf("amount cell type = %v, want numeric cell, not text", cellType)
	}
	styleID, err := out.GetCellStyle("Result_1", "C2")
	if err != nil {
		t.Fatal(err)
	}
	...
	if style.NumFmt != 2 {
		t.Fatalf("amount style NumFmt = %d, want copied NumFmt 2", style.NumFmt)
	}
}
